...
const Categories = () => {
    const { setActiveCategory } = useContext(adminContext)
    return (
        <div className='w-full flex flex-col justify-center items-center pt-[130px]' name="categories">
            ...
            <div className='w-[90%] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3  gap-6 items-center py-8'>
                {
                    productCategory.map((category) => (
                        <Card key={category.id} onClick={() => setActiveCategory(category.value)}
